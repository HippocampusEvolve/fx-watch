"""Отпечаток значения - половина механизма, на котором держатся требования 2 и 4.

Здесь проверяется только сам хэш: одинаковые значения дают одинаковый
отпечаток, изменённые - разный. Что с этим отпечатком делает база (уникальный
индекс, ``ON CONFLICT``, порядок версий, откат источника к прежнему значению),
проверяется в :mod:`tests.test_integration_db` на настоящем Postgres -
на заглушках такое утверждение доказать нельзя.
"""
