...
def _verdict(report: PeriodReport) -> str:
    ...
    * проверки не проходили, но всё, что они породили, уже закрыто (карантин
      разобран, алерты сняты, скачок курса на 12% - это рынок, а не сбой) -
      называем и то, и другое;
    * чисто везде - только тогда короткое «вмешательство не требуется».
    """
    ...
    # Проверки в периоде не проходили, но всё, что они породили, уже закрыто:
    # карантин разобран, алерты сняты. Это не повод дёргать человека, но и
    # молчать нельзя - иначе вердикт снова начнёт расходиться с разделом 5.
    notes = []
    if failed_blocking:
        notes.append(f"отклонённых значений: {failed_blocking} (карантин разобран)")
    if failed_warning:
        notes.append(f"предупреждений: {failed_warning} (данные приняты)")
    if notes:
        return (
            "Данные полные, все сбои восстановились автоматически; "
            + "непройденные проверки за период - " + ", ".join(notes)
