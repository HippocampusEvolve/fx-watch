...
class Settings(BaseSettings):
    ...
    # а не здесь: у ЦБ это около 54 валют, у источника для сверки - ровно три.
    ...
    # Свежесть меряется двумя линейками, потому что «данные перестали поступать» -
    # это два разных сбоя. freshness_hours - про сам сервис: как давно не было
    # успешного похода в источник (упал планировщик, сеть, предохранитель залип).
    # freshness_max_gap_days - про данные: сколько календарных дней подряд можно
    # прожить без единой новой даты. Мерить возраст последней вставки часами
    # нельзя: ЦБ не устанавливает курс на воскресенье и понедельник, штатная
    # тишина с субботы до вторника - 72 часа, а новогодняя - до 11 дней.
    freshness_hours: int = 36
    freshness_max_gap_days: int = 12
